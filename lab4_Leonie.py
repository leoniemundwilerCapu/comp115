...
assert find_max([1, 4, 5, 10]) == 10
assert find_max([9, 5, 12, 3, 1]) == 12
...
def evens(nums):
    ...
    for num in nums:
        if num % 2 == 0: 
            even_nums.append(num) # makes all in one list
    return even_nums
...
assert sum_of_squares([2, 4, 6, 7, 8]) == 169
